...
- [x] 1. 新建 `src/lib/types.ts`，迁移并扩展类型定义
...
- [x] 2. 迁移核心模块到 `src/lib/`
  - [x] 2.1 将 `src/llm-client.ts` 复制为 `src/lib/llm-client.ts`，更新 import 路径指向 `./types.js`
    - _Requirements: 1.2_
  - [x] 2.2 将 `src/model-resolver.ts` 复制为 `src/lib/model-resolver.ts`，更新 import 路径
    - _Requirements: 1.2_
  - [x] 2.3 将 `src/embedding-client.ts` 复制为 `src/lib/embedding-client.ts`，更新 import 路径
    - _Requirements: 1.2_

- [x] 3. 新建 `src/lib/config.ts`，提取配置加载逻辑
...
- [x] 4. 新建 `src/lib/chat.ts`，实现 `chat()` 主函数
...
- [x] 6. 检查点 - 确保所有测试通过，向用户确认无问题后继续

- [x] 7. 更新 `src/commands/chat.ts`，改为调用 lib/ 层
...
- [x]* 13. 编写属性测试（`vitest/pbt/`）
  ...
  - [x]* 13.2 编写 `vitest/pbt/config.pbt.test.ts`
...
- [x] 14. 最终检查点 - 确保所有测试通过，向用户确认无问题后继续
