...
describe('EmbeddingClient', () => {
  ...
  describe('resolveEndpoint', () => {
    ...
    it('should throw PAIError for unknown provider without baseUrl', () => {
      expect(() => EmbeddingClient.resolveEndpoint('azure-openai')).toThrow(PAIError);
      try {
  ...
  describe('embed – success', () => {
    it('should return embeddings for a single text', async () => {
      const apiResponse = makeOpenAIResponse([[0.1, 0.2, 0.3]]);
  ...
  describe('embed – API errors', () => {
    ...
    it('should throw PAIError with API_ERROR for 500', async () => {
      ...
      try {
        ...
        expect(e).toBeInstanceOf(PAIError);
        expect((e as PAIError).exitCode).toBe(ExitCode.API_ERROR);
        expect((e as PAIError).context?.status).toBe(500);
      }
    });
